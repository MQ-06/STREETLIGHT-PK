...
class BlockchainService:
    ...
    def register_complaint(
        ...
        """
        Blockchain Event #1 — ComplaintVerified
        Called after AI pipeline verifies a report.
        """
        ...
        try:
            ...
            image_hash_bytes32 = image_hash_bytes
            location_hash_bytes32 = location_hash_bytes
            ...
            logger.info(f"✅ BLOCKCHAIN: Complaint #{complaint_id} registered! Event: ComplaintVerified")
    ...
    def mark_resolved(
        ...
        """
        Blockchain Event #2 — ComplaintResolved
        Called after citizen confirms OR auto-close after 7 days.
        """
        if not self.enabled or self.contract is None:
        ...
        try:
            logger.info("=" * 50)
            logger.info(f"⛓️  BLOCKCHAIN: Resolving complaint #{complaint_id}")
            logger.info(f"   Note: {resolution_note}")
            logger.info("=" * 50)
            ...
            logger.info(f"✅ BLOCKCHAIN: Complaint #{complaint_id} RESOLVED! Event: ComplaintResolved")
            logger.info(f"   TX Hash:      0x{tx_hash.hex()}")
            logger.info(f"   Block Number: {receipt.blockNumber}")
    ...
    def get_complaint_proof(self, complaint_id: int) -> Dict:
        """Read-only — get complaint from chain. Public, no auth needed."""
        if not self.enabled or self.contract is None:
