...
#         # ==========================================
#         image_hash = generate_image_hash(temp_image_path)
...
#         # 1️⃣ Exact match first (fast path)
#         if image_hash:
#             existing_report = (
#                 db.query(Report)
...
#         # 2️⃣ If no exact match → similarity search
#         if existing_report is None:
#             all_reports = (
#                 db.query(Report)
#                 .filter(Report.image_hash.isnot(None))
#                 .all()
#             )

#             for r in all_reports:
#                 sim = hash_similarity(image_hash, r.image_hash)
...
def create_report(
    ...
    try:
        ...
        # Detect nearby-but-not-duplicate related issue (10–30 m zone)
        is_related = (
            fraud_result.get("duplicate_of_id") is None
            and fraud_result.get("duplicate_report") is not None
        )
